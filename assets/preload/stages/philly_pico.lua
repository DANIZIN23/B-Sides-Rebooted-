function onCreate()
    ...
    doTweenAlpha('luzAlpha', 'l', 0.8, stepCrochet*0.01, 'linear')
    setScrollFactor('l', 0, 0);
	addLuaSprite('l',true)

    makeLuaSprite('la', 'week3/luz azul', -900, -150)
    scaleObject('la', 1.1, 1.1)
    doTweenAlpha('luzAAlpha', 'la', 0.8, stepCrochet*0.01, 'linear')
...
function onBeatHit()
    ...
    if curBeat % 1 == 0 then
        doTweenAlpha('luzAlpha', 'l', 1, stepCrochet*2.50, 'linear')
        doTweenAlpha('luzAAlpha', 'la', 1, stepCrochet*2.10, 'linear')
	end
